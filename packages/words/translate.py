...
def get_en_audio_from_shanbay(content, cover_old=False):
    logger.debug('程序到达：translate.py-get_en_audio_from_shanbay函数')
...
def get_en_html(content):
    logger.debug('程序到达：translate.py-get_en_html函数')
...
def get_ch_json(content):
    logger.debug('程序到达：translate.py-get_ch_json函数')
    data = dict()
    data['type'] = 'AUTO'
    data['i'] = content
    data['doctype'] = 'json'
...
def get_en_audio(html, content, cover_old=False):
    logger.debug('程序到达：translate.py-get_en_audio函数')
...
def get_en_soundmark(html):
    logger.debug('程序到达：translate.py-get_en_soundmark函数')
    # 获取音标：得到的结果必须是字符串
    logger.info('正在抓取音标')
...
def parse_youdao_trans_en(html):
    logger.debug('程序到达：translate.py-parse_youdao_trans_en函数')
...
def get_en_translation_list(html):
    logger.debug('程序到达：translate.py-get_en_translation_list函数')
    # 获取译文：得到的结果必须是列表
    # 正确的英文都应该有译文(不一定有音标)
    logger.info('正在抓取译文')
    try:
...
def get_translation_en(content, cover_old=False):
    logger.debug('程序到达：translate.py-get_translation_en函数')
    # 最终必须以下面的结构返回
    result = [str(), list()]
...
def get_translation_ch(content):
    logger.debug('程序到达：translate.py-get_translation_ch函数')
...
def get_translation(content):
    logger.debug('程序到达：translate.py-get_translation函数')
    # 未输入任何内容情况下的处理
